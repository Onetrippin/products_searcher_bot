...
async def saved_page_changer(callback_query: types.CallbackQuery):
    _, page_type, current_page, _ = callback_query.data.split('_')
    await callback_query.message.edit_text(
        saved_message(callback_query.message, current_page),
        reply_markup=page_navigation_keyboard(page_type, 60, int(current_page))
    )

@router.callback_query(lambda call: call.data.startswith('page_history'))
async def history_page_changer(callback_query: types.CallbackQuery):
    _, page_type, current_page, _ = callback_query.data.split('_')
    await callback_query.message.edit_text(
...
async def page_counter(callback_query: types.CallbackQuery):
