    ...
    for product in products:
        ...
        for atom in atoms:
            ...
            if atom_type == 'priceV2':
                ...
                product_info['price'] = price[0].get('text').replace(' ', '')[:-1]
                if len(price) > 1:
                    product_info['orig_price'] = price[1].get('text').replace(' ', '')[:-1]
                    product_info['discount'] = atom_object.get('discount')
            elif atom_type == 'textAtom':
                product_info['title'] = atom_object.get('textAtom', {}).get('text')
        ...
        product_info['image'] = product.get('tileImage', {}).get('items')[0].get('image')
...
async def get_search_result(session: ClientSession, query: str) -> Tuple[str, list]:
